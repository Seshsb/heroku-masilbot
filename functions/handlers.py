...
def order(client, phone_number, method_pay, address, takeaway, lang):
    ...
    order_client += trans['delivery']['DELIVERY_ORDER_CLIENT_TOTAL_{}'.format(lang)].format(total, ). \
        replace(',', ' ') if not takeaway else trans['delivery']['DELIVERY_ORDER_ADMIN_TOTAL_{}'.format(lang)].format(
        total, ). \
        replace(',', ' ')
    return order_client
...
def accept_client(client, phone_number, method_pay, address, takeaway, lang):
    bot.send_message(client, order(client, phone_number, method_pay, address, takeaway, lang), parse_mode='html',
